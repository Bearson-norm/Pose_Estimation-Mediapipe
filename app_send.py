    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                # pos_landmark = process_pos_hand(pre_processed_landmark_list)
                # print(pos_landmark)
                
                x1, y1 = (pre_processed_landmark_list[0]),abs(pre_processed_landmark_list[1]) # palm
                x2, y2 = (pre_processed_landmark_list[8]),abs(pre_processed_landmark_list[9]) # thumb
                x3, y3 = (pre_processed_landmark_list[16]),abs(pre_processed_landmark_list[17]) # index
                x4, y4 = (pre_processed_landmark_list[24]),abs(pre_processed_landmark_list[25]) # middle
                x5, y5 = (pre_processed_landmark_list[32]),abs(pre_processed_landmark_list[33]) # ring
                x6, y6 = (pre_processed_landmark_list[40]),abs(pre_processed_landmark_list[41]) # little
                
                # ibu jari
                length = abs(math.hypot(x1-x2,y1-y2))
                # if length < 50:
                #     cv2.line(image,(x1,y1),(x2,y2),(0,0,0),3)
                # telunjuk
                length1 = abs(math.hypot(x1-x3,y1-y3))
                # if length1 < 50:
                #     cv2.line(image,(x3,y3),(x2,y2),(0,0,0),3)
                # jari tengah
                length2 = abs(math.hypot(x1-x4,y1-y4))
                # if length2 < 50:
                #     cv2.line(image,(x4,y4),(x2,y2),(0,0,0),3)
                # jari manis
                length3 = abs(math.hypot(x1-x5,y1-y5))
                # if length3 < 50:
                #     cv2.line(image,(x5,y5),(x2,y2),(0,0,0),3)
                # jari kelingking
                length4 = abs(math.hypot(x1-x6,y1-y6))
                # if length4 < 50:
                #     cv2.line(image,(x6,y6),(x2,y2),(0,0,0),3)
                # print(length, length1, length2, length3, length4)

                Pos = np.interp(length, [0, 1], [0, 100]) # ibu jari
                Pos1 = np.interp(length1, [0, 1], [0, 100]) # telunjuk
                Pos2 = np.interp(length2, [0, 1], [0, 100]) # jari tengah
...
def pre_process_landmark(landmark_list):
    ...
    # print(temp_landmark_list[0],temp_landmark_list[1])
...
# Callback function saat terhubung ke broker
def on_connect(client, userdata, flags, rc):
    print("Connected with result code " + str(rc))
    client.subscribe(topic)

# Callback function saat menerima pesan
def on_message(client, userdata, msg):
    print("Received message on topic: " + msg.topic + ". Message: " + str(msg.payload))
...
if __name__ == '__main__':
    ...
    # Start MQTT network loop (this is non-blocking)
    client.loop_start()
